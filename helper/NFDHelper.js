...
class NFDHelper {
    constructor() {
        this.face = new Face(new UnixTransport());
        this.keyChain = new KeyChain("pib-memory:", "tpm-memory:");
        this.keyChain.importSafeBag(new SafeBag
        (new Name("/testname/KEY/123"),
            new Blob(DEFAULT_RSA_PRIVATE_KEY_DER, false),
            new Blob(DEFAULT_RSA_PUBLIC_KEY_DER, false)));

        console.log('begin setCommandSigningInfo');
        try {
     ...
     */
    register(prefix, onInterest, onRegisterFailed = () => {
        console.log(`register ${prefix} failed`);
    }, onRegisterSuccess = () => {
        console.log(`register ${prefix} success`);
    }) {
        let name;
        if(prefix instanceof Name) {
            ...
            throw Error("prefix must be string or instance of Name")
        }

        this.face.registerPrefix(name, onInterest, onRegisterFailed, onRegisterSuccess);
    }
     ...
     * 给对应的Interest返回一个空数据
     *
     * 说明： 这个是在一个边界节点对另一个边界节点进行预请求（pre request）的时候，实际上可以不返回数据，但是如果返回一个Empty可以消除路径上对该
     *      预请求前缀的等待
     * @param interest 兴趣包
     */
    echoEmpty(interest) {
    ...
    /**
     * 发一个以name为名的interest包，或者直接发送一个指定的Interest包
     * @param nameOrInterest
     * @param onData
     * @param onError
     */
    expressInterest(nameOrInterest, onData = () => {}, onError = (e) => {console.log(e)}) {
        this.face.expressInterest(nameOrInterest, onData, onError);
    }

}

module.exports = NFDHelper;